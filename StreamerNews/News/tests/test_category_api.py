...
#                 categories_client = self.client.get(self.url,  {"Authorization": token})
...
#         for token, code in self.TOKENS_GET.items():
#             with self.subTest(f'{token}:{code}'):
#                 categories_client = self.client.get(self.url+"1/", {"Authorization": token})
#                 self.assertEqual(categories_client.status_code, code)
#                 self.assertEqual(categories_client.data["title"], "category1")
#
#     def test_post_category(self):
#         for token, code in self.TOKENS_POST.items():
#             with self.subTest(f'{token}:{code}'):
#                 categories_client = self.client.post(self.url, {"title": "SomeCategory"},
#                                                     headers={"Authorization": token})
#                 self.assertEqual(categories_client.status_code, code)
#         self.assertEqual(Category.objects.all().count(),3)
#         self.assertEqual(Category.objects.get(pk=3).title,"SomeCategory")
#
#     def test_delete_category(self):
#         for token, code in self.TOKENS_DELETE.items():
#             with self.subTest(f'{token}:{code}'):
#                 categories_client = self.client.delete(self.url+"1/",
#                                                       headers={"Authorization": token})
#                 self.assertEqual(categories_client.status_code, code)
...
#             with self.subTest(f'{token}:{code}'):
#                 categories_client = self.client.patch(self.url+"1/", data, content_type='application/json',
#                                                      headers={"Authorization": token})
#                 self.assertEqual(categories_client.status_code, code)
...
#             with self.subTest(f'{token}:{code}'):
#                 categories_client = self.client.put(self.url+"1/", data, content_type='application/json',
#                                                    headers={"Authorization": token})
#                 self.assertEqual(categories_client.status_code, code)
#         self.assertEqual(Category.objects.get(pk=1).title, "mynewtitle")